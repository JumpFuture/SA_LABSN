...
// @require      https://raw.githubusercontent.com/JumpFuture/SA_LABSN/main/anchor-browserified.js
// @require      https://raw.githubusercontent.com/JumpFuture/SA_LABSN/main/buffer-browserified.js
// @require      https://raw.githubusercontent.com/JumpFuture/SA_LABSN/main/bs58-browserified.js
...
(async function() {
	...
	let readRPCs = [
		'https://rpc.hellomoon.io/6d49be51-6ee8-4a9c-b918-9e3d23a2cacd',
...
*/				
	...
	const cargoProgramAddy = 'Cargo8a1e6NkGyrjy4BQEW4ASGKs9KSyDyUrXMfpJoiH';
	...
	const cargoProgramId = new solanaWeb3.PublicKey(cargoProgramAddy);
	...
	const r4Tokens = [
		...
		{name: 'Food', token: 'foodQJAztMzX1DKpLaiounNe2BDMds5RNuPC6jsNrDG'},
		{name: 'Ammo', token: 'ammoK8AkX2wnebQb35cDAZtTkvsXQbi82cGeTnUvvfK'},
	...
	cLog(2, 'cargoProgram', cargoProgram);
	cLog(2, 'cargoStatsDefinitionAcct', cargoStatsDefinitionAcct);
	...
	let [sduCargoTypeAcct] = await cargoProgram.account.cargoType.all([
			{
				 memcmp: {
						 offset: 41,
							bytes: new solanaWeb3.PublicKey(SDUAddy).toBase58(),
					},
			},
			{
				 memcmp: {
	...
	let [repairKitCargoTypeAcct] = await cargoProgram.account.cargoType.all([
			{
				 memcmp: {
						 offset: 41,
							bytes: new solanaWeb3.PublicKey(toolsAddy).toBase58(),
					},
			},
			{
				 memcmp: {
	...
	let [fuelCargoTypeAcct] = await cargoProgram.account.cargoType.all([
			{
				 memcmp: {
						 offset: 41,
							bytes: new solanaWeb3.PublicKey(fuelAddy).toBase58(),
					},
			},
			{
				 memcmp: {
	...
	let [sduTokenFrom] = await BrowserAnchor.anchor.web3.PublicKey.findProgramAddressSync(
			[
					...
					new solanaWeb3.PublicKey(SDUAddy).toBuffer()
			],
	...
	function createPDA(derived, derivedFrom1, derivedFrom2, fleet) {
			return new Promise(async resolve => {
					const keys = [{
							...
							pubkey: new solanaWeb3.PublicKey(tokenProgAddy),
							isSigner: false,
							isWritable: false
					}];
			...
			try {
					...
					for (let i = 0; i < gridSize; i++) {
							...
							for (let j = 0; j < gridSize; j++) {
									...
									let fleetAccts = await solanaReadConnection.getProgramAccounts(sageProgramId, {
											filters: [
	...
	async function execScan(fleet) {
			return new Promise(async resolve => {
					...
					let tx = { instruction: await sageProgram.methods.scanForSurveyDataUnits({keyIndex: new BrowserAnchor.anchor.BN(userProfileKeyIdx)}).accountsStrict({
							...
							cargoProgram: new solanaWeb3.PublicKey(cargoProgramAddy), // static
							tokenProgram: new solanaWeb3.PublicKey(tokenProgAddy), // static
	...
	async function execExitSubwarp(fleet) {
		return new Promise(async resolve => {
			let tx = { instruction: await sageProgram.methods.fleetStateHandler().accountsStrict({
					...
					{
							pubkey: new solanaWeb3.PublicKey(cargoProgramAddy),
							isSigner: false,
							isWritable: false
					},
					{
	...
	async function execWarp(fleet, destX, destY, moveTime) {
			return new Promise(async resolve => {
				let tx = { instruction: await sageProgram.methods.warpToCoordinate({keyIndex: new BrowserAnchor.anchor.BN(userProfileKeyIdx), toSector: [new BrowserAnchor.anchor.BN(destX), new BrowserAnchor.anchor.BN(destY)]}).accountsStrict({
							...
							cargoProgram: new solanaWeb3.PublicKey(cargoProgramAddy),
							tokenProgram: new solanaWeb3.PublicKey(tokenProgAddy)
					}).instruction()}
	...
	async function execCargoFromFleetToStarbase(fleet, fleetCargoPod, tokenMint, dockCoords, amount) {
			return new Promise(async resolve => {
					...
					let fleetCurrentPod = await solanaReadConnection.getParsedTokenAccountsByOwner(fleetCargoPod, {programId: new solanaWeb3.PublicKey(tokenProgAddy)});
					...
					let txResult = await txSignAndSend(tx, fleet, 'UNLOAD');
					resolve(txResult);
			});
	}
		...
		return new Promise(async resolve => {
			...
			for (let cargoHold of starbasePlayerCargoHolds) {
					if (cargoHold.account && cargoHold.account.openTokenAccounts > 0) {
							let cargoHoldTokens = await solanaReadConnection.getParsedTokenAccountsByOwner(cargoHold.publicKey, {programId: new solanaWeb3.PublicKey(tokenProgAddy)});
			...
			let [fleetRepairKitToken] = await BrowserAnchor.anchor.web3.PublicKey.findProgramAddressSync(
					[
							...
							new solanaWeb3.PublicKey(tokenProgAddy).toBuffer(),
							new solanaWeb3.PublicKey(toolsAddy).toBuffer()
					],
					new solanaWeb3.PublicKey(programAddy)
			);
	...
	async function execStopMining(fleet, sageResource, sageResourceAcctInfo, mineItem, resourceToken) {
			return new Promise(async resolve => {
					...
					let fleetCurrentCargo = await solanaReadConnection.getParsedTokenAccountsByOwner(fleet.cargoHold, {programId: new solanaWeb3.PublicKey(tokenProgAddy)});
	...
	async function execLoadFleetAmmo(fleet, starbaseCoords, amount) {
		...
		const parsedTokenAccounts = await solanaReadConnection.getParsedTokenAccountsByOwner(fleet.ammoBank, {programId: new solanaWeb3.PublicKey(tokenProgAddy)});
...
*/
			...
			for (let [i, row] of fleetRows.entries()) {
					...
					if (fleetAssignment !== '' && (warpCost > userFleets[userFleetIndex].fuelCapacity)) {
							...
							if (subwarpCost * 2 > userFleets[userFleetIndex].fuelCapacity) {
									cLog(1,'ERROR: Fleet will not have enough fuel to return to starbase');
			...
			if (readyToScan) {
					...
					if (!moved && Date.now() > userFleets[i].scanEnd) {
							...
							if (changesSDU.postBalance != changesSDU.preBalance) {
									...
									userFleets[i].scanSectorStart = scanCondition < userFleets[i].scanMin ? userFleets[i].scanSectorStart : Date.now();
							...
							//cLog(2, `${FleetTimeStamp(userFleets[i].label)} Tools: ${userFleets[i].toolCnt} | ${newToolCount}`);
							if(newToolCount < userFleets[i].scanCost)
							{
								...
								//cLog(2, `${FleetTimeStamp(userFleets[i].label)} Starting resupply`);
							}
					}
			} else {
				await handleResupply(i, fleetCoords);
	...
	async function handleResupply(i, fleetCoords) {
			...
			if (fleetCoords[0] == starbaseX && fleetCoords[1] == starbaseY) { //At starbase
					...
					cLog(1,`${FleetTimeStamp(userFleets[i].label)} Unloading`);
					...
					let fleetCurrentFuel = await solanaReadConnection.getParsedTokenAccountsByOwner(userFleets[i].fuelTank, {programId: new solanaWeb3.PublicKey(tokenProgAddy)});
					let currentFuelCnt = fleetCurrentFuel.value.find(item => item.pubkey.toString() === userFleets[i].fuelToken.toString())
					if (userFleets[i].scanCost > 0) {
							await execCargoFromStarbaseToFleet(userFleets[i], userFleets[i].cargoHold, userFleets[i].repairKitToken, toolsAddy, repairKitCargoTypeAcct, userFleets[i].starbaseCoord, userFleets[i].cargoCapacity - currentToolCnt.account.data.parsed.info.tokenAmount.uiAmount);
					}
					...
					let currentTool = fleetCurrentCargo.value.find(item => item.account.data.parsed.info.mint === toolsAddy);
	...
	async function handleTransport(i, fleetState, fleetCoords) {
			...
			if (fleetState === 'Idle') {
					...
					if (fleetCoords[0] == starbaseX && fleetCoords[1] == starbaseY) { 
							...
							if(!await handleTransportRefueling(userFleets[i], userFleets[i].starbaseCoord, moveDist, true, true, 0)) {
					...
					// Fleet at target?
						...
						//Refueling at Target
						if(!await handleTransportRefueling(userFleets[i], userFleets[i].destCoord, moveDist, false, false, fuelUnloadDeficit)) {
							userFleets[i].state = `ERROR: Not enough fuel`;
							return;
						}
	...
	async function getFleetFuelData(fleet, moveDist = 0) {
		const fleetCurrentFuelTank = await solanaReadConnection.getParsedTokenAccountsByOwner(fleet.fuelTank, {programId: new solanaWeb3.PublicKey(tokenProgAddy)});
		...
		return {
			...
			warpCost: Math.ceil(calculateWarpFuelBurn(fleet, moveDist)),
	...
	async function handleTransportRefueling(fleet, starbaseCoord, moveDist, fullTank = true, roundTrip = true, amountToDropOff = 0) {
		cLog(1,`${FleetTimeStamp(fleet.label)} ⛽ Refueling`);
		updateFleetState(fleet, 'Refueling');

		const fuelData = await getFleetFuelData(fleet, moveDist);
		...
		/*//Add extra fuel needed beyond fuel tank capacity
		const [fleetCargoFuelToken] = await BrowserAnchor.anchor.web3.PublicKey.findProgramAddressSync(
			[
					fleet.cargoHold.toBuffer(),
					new solanaWeb3.PublicKey(tokenProgAddy).toBuffer(),
					sageGameAcct.account.mints.fuel.toBuffer()
			],
			new solanaWeb3.PublicKey(programIdAddy)
		);

		const fleetCurrentCargo = await solanaReadConnection.getParsedTokenAccountsByOwner(fleet.cargoHold, {programId: new solanaWeb3.PublicKey(tokenProgAddy)});
		const cargoCnt = fleetCurrentCargo.value.reduce((n, {account}) => n + account.data.parsed.info.tokenAmount.uiAmount, 0);
		const cargoSpace = fleet.cargoCapacity - cargoCnt;
		if (fuelNeeded > fleet.fuelCapacity) {
			const currentFuel = fleetCurrentCargo.value.find(item => item.account.data.parsed.info.mint === sageGameAcct.account.mints.fuel.toString());
			const currentFuelCnt = currentFuel ? currentFuel.account.data.parsed.info.tokenAmount.uiAmount : 0;
			const fleetCargoFuelAcct = currentFuel ? currentFuel.pubkey : fleetCargoFuelToken;
			const cargoFuelAmt = calculateSubwarpFuelBurn(fleet, moveDist) - currentFuelCnt;

			if (cargoFuelAmt > 0 && cargoSpace > cargoFuelAmt) {
					cargoSpace -= cargoFuelAmt;
					cargoCnt += cargoFuelAmt;
					let fuelResp = await execCargoFromStarbaseToFleet(fleet, fleet.cargoHold, fleetCargoFuelAcct, sageGameAcct.account.mints.fuel.toString(), fuelCargoTypeAcct, fleet.starbaseCoord, cargoFuelAmt);
					if (fuelResp && fuelResp.name == 'NotEnoughResource') {
							cLog(1,`${FleetTimeStamp(fleet.label)} ERROR: Not enough fuel`);
							errorResource.push('fuel');
					}
	...
	async function handleTransportUnloading(fleet, starbaseCoord, transportManifest) {
		...
		//Ammo bank unloading
		const ammoEntry = transportUseAmmoBank ? transportManifest.find(e => e.res === ammoMint.toString()) : undefined;
		if (ammoEntry) {
			let fleetCurrentAmmoBank = await solanaReadConnection.getParsedTokenAccountsByOwner(fleet.ammoBank, {programId: new solanaWeb3.PublicKey(tokenProgAddy)});
	...
	async function handleTransportLoading(i, starbaseCoords, transportManifest) {
		...
		const ammoEntry = transportUseAmmoBank ? transportManifest.find(e => e.res === sageGameAcct.account.mints.ammo.toString()) : undefined;
		...
		for (const entry of transportManifest) {
			if (entry.res && entry.amt > 0) {
				...
				const fleetCurrentCargo = await solanaReadConnection.getParsedTokenAccountsByOwner(userFleets[i].cargoHold, {programId: new solanaWeb3.PublicKey(tokenProgAddy)});
				const cargoCnt = fleetCurrentCargo.value.reduce((n, {account}) => n + account.data.parsed.info.tokenAmount.uiAmount, 0);
	...
	function initUser() {
		return new Promise(async resolve => {
			...
			let userProfiles = await solanaReadConnection.getProgramAccounts(profileProgramId);
			let foundProf = [];
			...
			for (let fleet of userFleetAccts) {
				...
				let fleetCurrentFuel = await solanaReadConnection.getParsedTokenAccountsByOwner(fleet.account.fuelTank, {programId: new solanaWeb3.PublicKey(tokenProgAddy)});
